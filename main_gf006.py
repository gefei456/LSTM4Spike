...
# @Author:      WGF
# @File:        main_gf006.py
# @Description: point->point + tgpos + teaching force  for LFP

import torch
import torch.optim as optim
import numpy as np
import torch.nn as nn
from DataProcess.create_dataset_seq2seq import createDataset_teacher
import pickle
import time
import matplotlib
...
def train_GRU(dataloader, model, optimizer, loss_fn, label_state):
    ...
    total_loss = 0
    model.train()
    for i, (X, pos, vel, teach) in enumerate(dataloader):
        optimizer.zero_grad()
        pred = model(X, teach)[:, -1, :]
        if label_state == 'vel':
...
if __name__ == '__main__':
    ...
    encoder_layers = 2
    decoder_layers = 2
    session_name = 'qianqian_bj-co-mind-control_24-07-24_7_SBP_7_bip'
    bin_size = 0.05
    useBottleneck = False
    only4Test = False
    ...
    if isPkl:
        ...
        data_RefitNN = {'truth_traj': list(truth_traj), 'spike_list': spike_list,
                        'label_list': label_list, 'tgpos_list': tgpos_list}
    ...
    if not only4Test:
        ...
        ratio = [0.8, 0.2]
        train_dataset, test_dataset = createDataset_teacher(data_RefitNN, device, ratio)
