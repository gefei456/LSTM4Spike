...
class EncoderRNN(nn.Module):
    ...
    def initHidden(self, batch_size):
        result = torch.tensor(torch.zeros(1, batch_size, self.hidden_size))
        if use_cuda:
            return result.cuda()
        else:
            return result
...
def train_epoch(input_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion,
    ...
    encoder_outputs = torch.tensor(torch.zeros(max_length, batch_size, encoder.hidden_size))
    ...
    decoder_input = torch.tensor(torch.LongTensor([SOS_token] * batch_size))
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            ...
            decoder_input = torch.tensor(torch.cat(tuple(topi)))
...
def trainIters(encoder, decoder, epochs, train_loader, test_loader, max_length, learning_rate=0.01):
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_loader:
            loss = train_epoch(batch_x, batch_y, encoder,
...
def evaluate(encoder, decoder, loader, max_length):
    ...
    for batch_x, batch_y in loader:
        ...
        input_variable = torch.tensor(batch_x.transpose(0, 1))
        target_variable = torch.tensor(batch_y.transpose(0, 1))
...
if __name__ == "__main__":
    ...
    decoder1 = DecoderRNN(hidden_size, output_lang.n_words).cuda()
