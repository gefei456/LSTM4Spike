...
def test_refit_GRU(test_feat_list, test_tgpos_list, model, optimizer, loss_fn, label_state, alpha, bin_size):
    size = len(test_feat_list)
    num_batches = 0
    total_loss = 0
    model.train()
    # model.eval()
    ...
    for i, feat in enumerate(test_feat_list):
        ...
        tgPos = test_tgpos_list[i]
        x_pos = np.zeros(2,)
        for j, x in enumerate(X):
            optimizer.zero_grad()
            pred = model(x.unsqueeze(0))[:, -1, :].to('cpu').detach().numpy().squeeze(0)
            x_state = np.concatenate((x_pos, pred))

            # refit
            if label_state == 'all':
                print('only pos or vel cannot doing refit!!!')
                break
            state_new = refitVelocity(tgPos, x_state, alpha).reshape(-1).astype(np.float32)
            pred_new = state_new[2:]
            x_pos += pred_new * bin_size
            loss = loss_fn(torch.tensor(pred, requires_grad=True),
                           torch.tensor(pred_new, requires_grad=True))
            # loss = loss_fn(torch.tensor(x_state[2:], requires_grad=True),
            #                      torch.tensor(pred_new[2:], requires_grad=True))
            ...
            num_batches += 1
            if i % 5 == 0:
                refitloss, current = loss.item(), (i + 1) * len(X)
                print(f"loss: {refitloss:>7f}  [{current:>5d}/{size:>5d}]")
    if num_batches != 0:
        refit_loss = total_loss / num_batches
    else:
        refit_loss = total_loss
    return refit_loss


def train_refit(test_feat_list, test_tgpos_list, model, optimizer, loss_fn, alpha, bin_size):
    refit_loss = test_refit_GRU(test_feat_list, test_tgpos_list, model, optimizer, loss_fn, label_state,
                                alpha, bin_size)
    print('refit_loss', refit_loss)
    print("refit model Done!")
...
if __name__ == '__main__':
    ...
    useBottleneck = False
    only4Test = False
    time_str = 'GRU_refit_2023-09-26--13_48_04_0.05_7_-0.0_noAlign_13'
    seed = 13
    ...
    r2_all /= 8
    dist_all /= 8
    plt.title(f'GRU+base-{session_name}-{local_time}-r2:{r2_all: .3f}-dist:{dist_all: .3f}')
    plt.show()

    # train refit
    test_feat_list = data_RefitNN['spike_list'][200:360]
    test_tgpos_list = data_RefitNN['tgpos_list'][200:360]
    loss_fn = nn.MSELoss()
    optimizer_2 = optim.Adam(model.parameters(), lr=0.01, betas=(0.9, 0.999), eps=1e-08, weight_decay=0.01)
    train_refit(test_feat_list, test_tgpos_list, model, optimizer_2, loss_fn, alpha, bin_size)
    ...
    r2_all /= 8
    dist_all /= 8
    plt.legend(['truth trajectory', 'predicted trajectory'])
    # plt.title(f'GRU+refit-{session_name}-{local_time}-r2:{r2_all: .3f}-dist:{dist_all: .3f}')
    plt.title("'center-out'experiment")
    plt.show()
