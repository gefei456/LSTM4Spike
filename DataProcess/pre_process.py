...
import global_pos_calc as gpc
import math
from trial_util import trial_cell_to_dict as c2d
import os
import pickle
...
if __name__ == '__main__':
    ...
    if DataTypeName == 'Python':
        ...
        dataPath = file_path + '20230209_3.mat'
...
# 数据分组
    ...
    if DataTypeName == 'Python':
        ...
        for pos in truthPointsFixed:
            if pos[1] != 6000:
                # 坐标转换
                pos_new = gpc.pos_screen2std(pos[1:3], screenWeight, screenHeight)
                pos[1:3] = pos_new
...
# 保存
    file_to_save = {"truth_traj": X_Truth_list, "spike_list": X_Feature_list}
    with open(f"{session_name}_{bin_size}_ReNN.pkl", "wb") as f:
        pickle.dump(file_to_save, f)
    # np.save(f"{session_name}_ReNN.npy", file_to_save)
    print(f'成功保存{session_name}.pkl文件!')

# 读取
    with open(f"{session_name}_{bin_size}_ReNN.pkl", "rb") as f:
